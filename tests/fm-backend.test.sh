...
# fm-stat-lib.sh is a leaf sibling with no dependencies of its own, but several
# copied siblings below (fm-classify-lib.sh, fm-supervision-lib.sh,
# fm-config-inherit-lib.sh) now source it by BASH_SOURCE-relative path, which
# resolves inside THIS fake bin/ - so it has to be copied here too or they abort.
OLD_BIN_UNCHANGED_SIBLINGS="fm-gate-refuse-lib.sh fm-guard.sh fm-lock-lib.sh fm-tasks-axi-lib.sh fm-pr-lib.sh fm-tangle-lib.sh fm-tmux-lib.sh fm-composer-lib.sh fm-wake-lib.sh fm-stat-lib.sh fm-classify-lib.sh fm-supervision-lib.sh fm-ff-lib.sh fm-config-inherit-lib.sh fm-project-mode.sh fm-harness.sh fm-crew-state.sh fm-nm-run-lib.sh fm-decision-hold.sh fm-backend.sh fm-operational-input.sh fm-public-followup-lib.sh fm-secondmate-registry-lib.sh fm-secondmate-parent-lib.sh fm-x-lib.sh"
