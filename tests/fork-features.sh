...
echo "Testing: Beads Integration"
  { [[ -f bin/fm-brief-hooks.d/beads.sh ]] && [[ -x bin/fm-brief-hooks.d/beads.sh ]]; } && test_pass || test_fail "bin/fm-brief-hooks.d/beads.sh executable"
  ...
  { [[ -d bin/fm-spawn-hooks ]] && [[ -f bin/fm-spawn-hooks/beads ]] && [[ -x bin/fm-spawn-hooks/beads ]]; } && test_pass || test_fail "bin/fm-spawn-hooks/beads executable"
...
echo "Testing: Decision Hold Lifecycle"
  ...
  { [[ -f bin/fm-session-start.sh ]] && [[ -x bin/fm-session-start.sh ]] && grep -q 'OPEN DECISIONS\|open_decisions' bin/fm-session-start.sh; } && test_pass || test_fail "OPEN DECISIONS display (fm-session-start.sh executable)"

echo ""
echo "Testing: Beads Task-Store Backend"
  { [[ -f bin/fm-backlog-handoff.sh ]] && [[ -x bin/fm-backlog-handoff.sh ]]; } && test_pass || test_fail "bin/fm-backlog-handoff.sh executable"
  { [[ -f bin/fm-backlog-receive.sh ]] && [[ -x bin/fm-backlog-receive.sh ]]; } && test_pass || test_fail "bin/fm-backlog-receive.sh executable"
...
echo "Testing: X-mode Integration"
  ...
  { [[ -f bin/fm-x-lib.sh ]] && [[ -x bin/fm-x-lib.sh ]]; } && test_pass || test_fail "bin/fm-x-lib.sh executable"
...
echo "Testing: Herdr Backend Support"
  { [[ -f bin/backends/herdr.sh ]] && [[ -x bin/backends/herdr.sh ]]; } && test_pass || test_fail "bin/backends/herdr.sh executable"
