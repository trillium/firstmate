...
#   (ax) a merge poll that cannot be armed still merges and reports separately
#   (ay) a check that fails before recording pr= refuses and says no merge ran
#   (az) a rate-limited CodeRabbit (green check, zero reviews) refuses the merge
#   (ba) a real CodeRabbit review lets the merge through
#   (bb) an unreadable CodeRabbit review state refuses rather than merging
#   (bc) FM_CODERABBIT_GATE=skip waives the gate deliberately
#   (bd) the CodeRabbit verdict script classifies reviewed/rate-limited/pending
set -u
...
run_pr_merge() {
  ...
  FM_CODERABBIT_GATE="${FM_CODERABBIT_GATE-skip}" \
...
test_rate_limited_coderabbit_refuses_merge() {
  ...
  set +e
  FM_CODERABBIT_GATE=enforce run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/67 \
    > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
...
JSON
  ...
  FM_CODERABBIT_GATE=enforce run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/68 \
    > "$case_dir/stdout" 2> "$case_dir/stderr" \
...
test_unreadable_review_state_refuses_merge() {
  ...
  set +e
  FM_CODERABBIT_GATE=enforce run_pr_merge "$case_dir" task-x1 https://github.com/example/repo/pull/69 \
    > "$case_dir/stdout" 2> "$case_dir/stderr"
  rc=$?
  set -e
