...
fm_lint_worker() {  # <manifest> <output-dir> <shard-index>
  ...
  # --fast is the local-only knob: ShellCheck extended analysis is the slowest
  # part of a full lint, and CI always runs without it (see the --fast guard).
  ...
  while IFS="$tab" read -r index entry path || [ -n "${index:-}" ]; do
    ...
    root_rc=0
    fm_lint_slot_acquire
    "$FM_LINT_SHELLCHECK" "${shellcheck_args[@]}" -- "$path" > "$output_dir/raw.$index" 2>&1 &
    FM_LINT_WORKER_SHELLCHECK_PID=$!
