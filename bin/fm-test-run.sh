...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-timeout-lib.sh)
      ...
      # wedge detector's worktree write probe, and the landing predicate's remote
      # legs whenever a caller opts into a bound - which reaches
      # bin/fm-teardown.sh's landing check (pr-forge) too.
      printf '%s\n' session-bootstrap
      printf '%s\n' snapshot-bearings
      printf '%s\n' pure-contract-unit
      printf '%s\n' secondmate
