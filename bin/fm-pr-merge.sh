...
#
# Before recording anything, a GitHub merge is gated on CodeRabbit having actually
...
coderabbit_gate() {
  local state rc=0
  # CodeRabbit reviews GitHub pull requests. A GitLab merge request is gated on
  # its own live pre-merge conditions instead, so it never reaches this gate.
...
# Recording the PR is the prerequisite for merging; arming the watcher's merge
# poll is not. Status 3 means the metadata was recorded and only the arming
# failed, so the merge still runs and the arming failure is reported on its own,
# above the merge output and again below it. Any other non-zero status refuses
# and says plainly that no merge was performed.
ARMING_WARNING=

report_arming_warning() {
...
record_pr_metadata() {
  ...
  case "$check_rc" in
    ...
    3)
      ...
      printf '%s\n' "$ARMING_WARNING" >&2
      # Repeat it on every path out of this script, so a long forge report
      # cannot bury the one line that says supervision is not watching.
      trap report_arming_warning EXIT
      ;;
    *)
      echo "error: no merge was performed: recording the PR for $ID failed (bin/fm-pr-check.sh exit $check_rc); no merge command was called" >&2
      return "$check_rc"
      ;;
  esac
