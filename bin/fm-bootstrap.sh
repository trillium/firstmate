...
# A configured cursor crew harness needs a cursor executable present, and cursor
# ships under EITHER installed name. Resolution runs through the verified owner
# rather than a bare `command -v`, so a home that merely has some unrelated
# executable named `agent` on PATH is still reported missing instead of failing
# at the first spawn.
if local_phase && [ "$crew" = cursor ] && ! fm_cursor_resolve_binary >/dev/null 2>&1; then
...
if [ "${FM_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ]; then
  ...
  # Clone refresh does not depend on them, so it starts in the background and
  # overlaps their wall clock.
  fleet_sync_pid=
  fleet_sync_out=
