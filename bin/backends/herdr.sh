...
# naming"). Herdr pane labels are display-only: they never authorize lookup,
# adoption, reuse, closure, task ownership, or endpoint selection. Tab labels
# (fm-<id>) and workspace labels remain the only label-based task identity.
# Every label reads "<harness> <agent> <label>": an owner pane carries the
# active harness plus its role and a crown, for example "Pi Firstmate 👑"
# and "Pi TalonMate 👑"; a subordinate worker pane carries the active
# harness plus its owning mate plus its linked task id, for example
# "Pi TalonMate · task-d6376", and never a crown. The functions below are
# pure except the two that read or rename a live pane, and the rename path
# preserves any captain-set label it does not recognize.
...
# resolves through its marker id, with TalonMate pinned and any other id
# Titlecased from its sanitized scope so a future secondmate still gets a
# stable distinct role.
fm_backend_herdr_owner_role_for_home() {
  local home=${1:-} marker id lower scope first rest
  [ -n "$home" ] || home=${FM_HOME:-}
  [ -n "$home" ] || { printf 'Firstmate'; return 0; }
  marker="$home/$FM_BACKEND_HERDR_SECONDMATE_MARKER"
  if [ ! -f "$marker" ]; then printf 'Firstmate'; return 0; fi
  id=$(cat "$marker" 2>/dev/null)
  id="${id#"${id%%[![:space:]]*}"}"
  id="${id%"${id##*[![:space:]]}"}"
  ...
  case "$lower" in
    ...
    talonmate|talon-mate|talon_mate|2m-talonmate|2m-talon-mate|2m-talon_mate) printf 'TalonMate'; return 0 ;;
  esac
  scope=$(fm_backend_herdr_mate_scope "$id")
  lower=$(printf '%s' "$scope" | tr '[:upper:]' '[:lower:]')
...
# <agent> <task-id>. Never contains a crown by construction; the owning
# mate stays visible between the harness and the task id.
fm_backend_herdr_worker_pane_label() {
  local harness=${1:-} agent=${2:-} task=${3:-} display
  [ -n "$harness" ] && [ -n "$agent" ] && [ -n "$task" ] || return 1
  case "$agent" in
    *[[:space:]]*|*"·"*|*"👑"*|*[!A-Za-z0-9-]*) return 1 ;;
  esac
  case "$task" in
  ...
  display=$(fm_backend_herdr_harness_display "$harness") || return 1
  printf '%s %s · %s' "$display" "$agent" "$task"
}
...
fm_backend_herdr_pane_label_is_managed() {
  local label=${1:-} display rest body agent task
  [ -n "$label" ] || return 1
  case "$label" in
    fm-*) return 0 ;;
  esac
  display=${label%%" "*}
  [ "$display" != "$label" ] || return 1
  fm_backend_herdr_harness_display_is_known "$display" || return 1
  rest=${label#*" "}
  case "$rest" in
    *"👑"*)
      case "$rest" in
...
# so labeling can never fail a spawn or steal a turn.
...
# convention and may read "<harness> <agent> · <id>", while the tab keeps
# fm-<id> as the label-based identity. Returns 0 if verified, 1 otherwise.
# This is used to self-repair legacy metadata lacking endpoint_task_id.
