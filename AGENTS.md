...
config/crew-account  optional default Claude account index for spawns that pass no --account; LOCAL, gitignored; one positive integer (docs/configuration.md "Multi-account Claude Code")
...
state/               volatile runtime signals; gitignored
  ...
  <id>.meta          written by fm-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); kind=secondmate also records home= and projects=, plus remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for a remote route; a claude-harness spawn records account= when --account was passed at spawn or, absent that flag, config/crew-account holds a positive integer supplying the default (docs/configuration.md "Multi-account Claude Code"); a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/fm-backend.sh, section 8); optional claim_prompt=on is recorded only when a launch actually used the optional Parlay claim prompt, and is re-decided rather than inherited on --relaunch (docs/configuration.md "Parlay claim prompt"); optional label= is recorded only when --label was passed at spawn; optional beads_id= is recorded when --beads was passed at spawn or, under config/backlog-backend=beads, auto-resolved/minted for every non-secondmate spawn (bin/fm-tasks-axi-lib.sh's fm_beads_resolve_or_create), driving the claim-first/close-last bead lifecycle (bin/fm-brief-hooks.d/beads.sh, bin/fm-teardown.sh header, bin/fm-ledger.sh) under section 7's bead-linked dispatch; fm-pr-check, including through fm-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); fm-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for a Relay-originated task (section 14)
