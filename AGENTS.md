...
7. **Network checks** - the `NETWORK CHECKS` section reports the deferred non-blocking harvest of the four network sweeps started at step 1 (GitHub auth, project clone refresh, secondmate liveness and convergence, pending handoff delivery), printing whatever the worker has published by now and naming the rest as not yet confirmed.
8. **Persona** - the full contents of the active persona file (`config/persona.md` when present, else tracked `persona.md`), always printed regardless of lock state - including lock-refused read-only mode - after the digests, so the captain-facing voice is reliably in force.
