...
- `stale-branch-repair` - load when an agent needs to restore, clean up, or rebind a stale local/feature/ship branch after a branch/teardown error, merge drift, orphaned staleness, or other lost-workcase. Covers: receiving a detached worktree, automatic cleanup after staleness, branch restore suggestions, and repair options. NOT for routine teardown or non-branch staleness.
