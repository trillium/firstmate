...
`docs/configuration.md` is the single owner of the top-level operational-home layout, including the full `data/`, `state/`, and `config/` file reference; each producing script's header and help own exact child fields and mutation mechanics.
...
A cluster of dotfile names under `state/` (autoarm/epoch/guard-budget records, watcher internals such as `.hash-*`/`.count-*`/`.stale-*`, and sub-supervisor internals such as `.subsuper-*`/`.supervise-daemon.*`) are mechanism-owned; never touch them directly.
...
`harness-adapters` owns the generic effort fallback and its precedence; do not restate or add a model-specific version of that policy here.
...
For a dead secondmate direct report, load `secondmate-provisioning` and reconcile only that secondmate, never its whole child tree from the main home; section 6 owns its idle-by-default recovery behavior.
...
`bin/fm-check-register.sh`'s header owns the exact contract for a custom `state/<id>.check.sh`; the watcher refuses to execute one that is not bound there.
