...
Treat a task whose endpoint has gone quiet with no captain-facing wake as a possible automatic reclaim rather than a crash; `state/<id>.status` and the recorded worktree still hold the evidence.
