...
4. **Persona** - the full contents of the active persona file (`config/persona.md` when present, else tracked `persona.md`), printed every session regardless of lock state - including lock-refused read-only mode - so the captain-facing voice is reliably in force before the context and fleet-state digests.
