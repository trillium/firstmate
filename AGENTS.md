...
- `stale-branch-repair` - load before briefing any rebase, conflict-resolution, or "PR is conflicting" task, and before accepting a crewmate's report that a conflicting branch was fixed. Owns contribution-first triage (rebase vs rebuild vs close), the four-marker diff3 hazard, firstmate-escalation for semantic conflicts, and repo-derived verification that replaces trusting a worker's summary. NOT for branch teardown, detached worktrees, or non-conflict staleness.
