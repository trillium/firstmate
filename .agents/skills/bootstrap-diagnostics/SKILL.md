...
- `BEADS_SYNC: commit failed: 'task dolt commit' exited <n>: <reason>` - unlike the lines above, this one is local, not remote: the commit step found work to commit and could not commit it (a Dolt error, schema skew, or a permission problem), so this home's writes are still sitting uncommitted in the Dolt working set and any push that follows did not carry them.
  A clean working set is never reported here, so this line always means real uncommitted work is stranded; investigate the named error rather than waiting for the next sweep, because backing off does not clear it.
