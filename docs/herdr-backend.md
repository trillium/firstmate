...
Every label reads `<harness> <agent> <label>`: an owner pane shows the active harness plus its role and a crown, for example `Pi Firstmate 👑` and `Pi TalonMate 👑`.
A subordinate worker pane never uses a crown and shows the active harness plus its owning mate plus its linked task identifier, for example `Pi TalonMate · task-d6376`, so the parent relationship stays visible.
