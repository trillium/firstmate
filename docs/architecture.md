...
Ahead of that run attribution, and only under the beads backlog backend, a closed linked bead short-circuits the read as a lifecycle-derived `done`; an open bead changes nothing, so live work still resolves through the run-step and pane logic ([configuration.md](configuration.md) owns that completion-side rule under "Backlog backend").
