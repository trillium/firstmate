...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR polling (merge or automated-reviewer submissions) or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS` without their own task worktree being written, declared external waits and verified captain-held transfers that remain declared past `FM_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
When a canonical validated PR poll returns `merged`, the watcher routes it through the shared merge-outcome emitter before retiring the poll.
A `bot-review` result (GitHub only) instead signals a new automated-reviewer submission and keeps the poll armed for further reviews and the eventual merge.
