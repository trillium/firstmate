...
The hint table merges two green runs, because this fork and upstream measured different script sets.
This fork contributed the per-shard `fm-test-timing-portable-serial-*` artifacts of run 32589886005 on 2026-08-22, where the lane ran 130 scripts in 2367147 ms of serial work, and owns the hint wherever both runs measured the same script.
Upstream contributed the same artifacts of green CI run [32491999845](https://github.com/kunchenguid/firstmate/actions/runs/32491999845) on 2026-08-21, where the lane ran 116 scripts in 2541548 ms, for the 18 scripts this fork had not yet measured.
...
Scripts added since either run carry no hint and pack as zero until the next remeasurement.
...
| Lane | Script count | Estimated duration |
|---|---:|---:|
| `portable-serial-1of4` | 40 | 653851 ms (~653.9 s) |
| `portable-serial-2of4` | 41 | 653853 ms (~653.9 s) |
| `portable-serial-3of4` | 41 | 673842 ms (~673.8 s) |
| `portable-serial-4of4` | 41 | 653833 ms (~653.8 s) |
| imbalance | | 20009 ms |
