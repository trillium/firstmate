...
The fleet-facing `bin/fm-fleet-view.sh` and `bin/fm-remote-secondmate-control.sh` are the opposite exception: an unset `FM_HOME` resolves to the conventional operational home `$HOME/data/firstmate` rather than the repo root, and they fail with a path-naming diagnostic only when that defaulted home does not exist; an explicitly set `FM_HOME` is honored unchanged, and their `--help` and unknown-command paths never resolve a home.
