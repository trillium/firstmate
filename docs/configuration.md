...
A replacement persona file should still define an address term, a seasoning policy including when to drop it, and the routine acknowledgment phrase that `AGENTS.md` section 9 references, so those functional rules have a voice to apply.
...
There is deliberately no on-demand skill for the persona: one that loaded only when asked for could be skipped, and the voice is meant to be active on every response.
