...
When another worktree of the same repository already has the default branch checked out, git cannot give it to the primary as well, so bootstrap prints a `BOOTSTRAP_INFO:` fact naming that holder instead of a `TANGLE:` line and no checkout remediation is offered.
