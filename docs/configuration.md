...
A failed mint or a failed record rewrite on the repointing path counts as a candidate failure the same way, so a task is never left silently pinned to another home's bead.
