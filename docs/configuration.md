...
A one-way compatibility read (`fm_beads_lookup`) still finds a pre-migration bead carrying the legacy unscoped `task:<task_id>` label, but only when the bead is open, this home's own `state/<task_id>.meta` already records that bead id as its `beads_id=`, and the bead carries no other home's `task:<scope>:<task_id>` label; that owned bead is then re-tagged with the scoped label exactly once rather than re-minted, and other homes' beads are never rewritten.
Both ownership tests are needed because a pre-migration collision wrote the same bead id into both homes' records, so the local record alone proves only that this home resolved to the bead, not that it is the only home that did: the first home to migrate keeps the shared bead and stamps its scope on it, the second sees that foreign scope and mints its own, and the two stop sharing.
A legacy bead whose labels cannot be read is not adopted either, since an unreachable store is not evidence that no other home has claimed it.
The legacy read is only attempted when `state/<task_id>.meta` already records a `beads_id=`, so a brand-new task costs one store call rather than two.
