...
When two homes both hold a claiming record for one slug and only one pre-migration bead exists, the label cannot say which home minted it, so the first home to sweep claims that bead and the second mints its own on its next resolve: both homes end with one bead each, which is the property that matters, though the older bead's history may end up under the other home.
