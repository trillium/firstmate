...
FM_STALENESS_FOCUS_GRACE_SECS=300        # herdr only: seconds a reap-candidate pane stays protected from the staleness auto-close reclaim after a human last focused it; a currently- or recently-focused pane blocks the reclaim without consuming the retry budget, and an unreadable focus also blocks but only until FM_STALENESS_AUTOCLOSE_MAX_RETRIES consecutive unreadable polls (any readable poll resets the count), after which a dead pane falls through to reap rather than being suppressed forever
