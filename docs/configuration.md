...
A one-way compatibility read (`fm_beads_lookup`) still finds a pre-migration bead carrying the legacy unscoped `task:<task_id>` label, but only when the bead is open and this home's own `state/<task_id>.meta` already records that bead id as its `beads_id=` - the only sound ownership signal, since the legacy label records no home - and that owned bead is then re-tagged with the scoped label exactly once rather than re-minted; other homes' beads are never rewritten.
