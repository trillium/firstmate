...
| `fm-stat-lib.sh`         | Shared portable-`stat` dialect owner: feature-detect the `stat` binary's dialect (BSD vs GNU) once per process and dispatch accordingly, instead of keying on `uname`; migration of remaining `stat` call sites onto it is in progress |
