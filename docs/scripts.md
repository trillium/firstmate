...
| `fm-backlog-handoff.sh`  | Validate and delegate queued backlog-item moves into a secondmate home, then durably wake its recorded receiver |
...
| `fm-send.sh`             | Steer a task via a durable inbox record plus doorbell, or send a supported key or typed harness invocation through the recorded backend; `--raw` sends unverified best-effort to a raw/unmanaged pane |
...
| `fm-pr-merge.sh`         | Record PR metadata prerequisite, merge a task's canonical full GitHub or GitLab URL, then refuse an outcome it cannot prove landed or queued; reports arming failure separately when status 3 |
